...
<xsl:stylesheet xmlns:guiao="urn:guiao" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="hierarquia:cena">
        ...
        <xsl:choose>
            <xsl:when test="$depth = 1">
                <div class="page">
                    <div class="subpage">
                        <xsl:element name="h{$depth}"> Cena <xsl:value-of select="@num-cena"/> -
                                <span class="context">
            ...
            <xsl:otherwise>
                <xsl:element name="h{$depth}"> Cena <xsl:value-of select="@num-cena"/> - <span
                        class="context">
                    <xsl:value-of select="$contexto"/>
                    </span>
                </xsl:element>
    ...
    <xsl:template match="hierarquia:fala">
        <xsl:element name="p">
            <span class="uppercase" ><xsl:value-of select="key('ref-personagem', @ref-personagem)"/></span>
        </xsl:element>
        <xsl:element name="p">
    ...
    <xsl:template match="hierarquia:adereco">
        <span class="prop"><xsl:value-of select="key('ref-adereco', @ref-adereco)"/></span>
    </xsl:template>
