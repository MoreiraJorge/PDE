...
<xsl:stylesheet version="1.0" xmlns:hierarquia="urn:hierarquia" xmlns:tipo="urn:tipo"
  ...
  <xsl:template match="hierarquia:fala" mode="validate-references">
    <xsl:param name="numCena"/>
    <xsl:if test="not(local-name(key('ref-personagem', @ref-personagem)) = 'personagem')">
      <xsl:message terminate="no">O elemento fala na cena número <xsl:value-of select="$numCena"/>
        contém uma referência inválida para uma personagem ( ref-personagem = "<xsl:value-of
          select="@ref-personagem"/>" ).</xsl:message>
    </xsl:if>
  ...
  <xsl:template match="hierarquia:refere" mode="validate-references">
    <xsl:param name="numCena"/>
    <xsl:if test="not(local-name(key('ref-personagem', @ref-personagem)) = 'personagem')">
      <xsl:message terminate="no">O elemento refere na cena número <xsl:value-of select="$numCena"/>
        contém uma referência inválida para uma personagem ( ref-personagem = "<xsl:value-of
          select="@ref-personagem"/>" ).</xsl:message>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="hierarquia:adereco" mode="validate-references">
    ...
    <xsl:if test="not(local-name(key('ref-adereco', @ref-adereco)) = 'adereco')">
      <xsl:message terminate="no">O elemento adereco na cena número <xsl:value-of select="$numCena"
        /> contém uma referência inválida para um adereço ( ref-adereco = "<xsl:value-of
          select="@ref-adereco"/>" ).</xsl:message>
    </xsl:if>
  </xsl:template>
